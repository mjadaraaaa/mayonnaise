...
class ActivityController extends Controller
{
    public function addActivity(Request $request)
    {
        try {
            ...
            return response()->json([
                "status" => "error",
                "message" => "an error occured while adding an activity",
            ]);
        }
    }
    ...
    public function deleteActivity($activity_id)
    {
        try {
            ...
            return response()->json([
                ...
                "status" => "error",
                "message" => "There was an error while trying to delete activity",
    ...
    {
        try {
            $activity = Activity::with('user')->with('city')->with("activityPictures")->findorfail($activity_id);
            return response()->json([
                "status"=>"success",
    ...
    public function getAllActivities()
    {
        try {
            ...
            $activities = Activity::with('user')->with('city')->with("activityPictures")->get();
    ...
    public function nancySearch(Request $request)
    {
        try {
            $search = "";
                ...
                $search = $request->activities;
                $activities = Activity::whereHas('city', function ($query) use ($search) {
                $query->where('city_name', 'like', "%{$search}%");
                })->get();
                
                
                if($request->restaurants){
                $search = $request->restaurants;
                $activities = Activity::whereHas('city', function ($query) use ($search) {
                $query->where('city_name', 'like', "%{$search}%");
                })->whereHas('activityType', function ($query)  { $query->where("name","restaurants");})->get();
                }   
                ...
                if($request->hotels){
                $search = $request->hotels;
                $activities = Activity::whereHas('city', function ($query) use ($search) {
                $query->where('city_name', 'like', "%{$search}%");
                })->whereHas('activityType', function ($query)  { $query->where("name","hotels");})->get();
                }  
    ...
    public function randomActivities()
{
    try{
  $activities = Activity::all();
  $randomActivities = $activities->shuffle()->take(10); 
